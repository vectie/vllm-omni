...
| HiFT stage-0 residual-block graphs | Implemented, opt-in; three exact TorchAir graphs are 2.158x faster in isolation, while the three-run 32-row service median improved duration 5.67%, throughput 6.02%, mean chunk RTF 5.66%, TTFT 1.35%, and TTFP 2.26%; P99 TTFP regressed 0.71%, and a nine-graph all-stage candidate was rejected at 4.09% slower than control despite exact 2.08-2.16x isolated stage wins; full competition qualification remains required |
...
| Aggregate three-sibling HiFT graph | Rejected and removed; exact output and 3.087x isolated speedup, but the fully resident 32-row run regressed duration 11.75%, mean chunk RTF 10.44%, TTFT 9.62%, and TTFP 7.65% versus the accepted three-graph profile |
